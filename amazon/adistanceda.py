...
parser.add_argument("-dir", "--directory", default="./checkpoints/out/da2")
...
def alpha_distance(trainer, source_loader, target_loader, device, path):
    ...
    with torch.no_grad():
        for index, batch in enumerate(source_loader):
            inputs = batch[0].to(device)
            pred = trainer.model.encoder(inputs)[1]
...
if __name__ == '__main__':
    ...
    bertmodel = BertModel.from_pretrained('./okit')
    ...
    from slp.modules.vada import VADAClassifier
